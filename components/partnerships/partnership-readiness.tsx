import { HomeSection, SectionHeader } from "@/components/home/section";
import { cn } from "@/lib/utils";
import {
  type ReadinessStatus,
  readinessStatusStyles,
} from "./partnerships-links";

const readinessItems: {
  label: string;
  status: ReadinessStatus;
  statusLabel: string;
}[] = [
  { label: "Public partnerships page", status: "active", statusLabel: "Active" },
  { label: "Platform overview", status: "active", statusLabel: "Active" },
  { label: "Developer foundation", status: "active", statusLabel: "Active" },
  {
    label: "Security and governance foundation",
    status: "active",
    statusLabel: "Active",
  },
  {
    label: "Merchant partnership pathway",
    status: "in-progress",
    statusLabel: "In development",
  },
  {
    label: "Financial partner pathway",
    status: "planned",
    statusLabel: "Planned",
  },
  {
    label: "Partner onboarding workflow",
    status: "planned",
    statusLabel: "Planned",
  },
  {
    label: "Production partner expansion",
    status: "future",
    statusLabel: "Future phase",
  },
];

export function PartnershipReadiness() {
  return (
    <HomeSection
      className="bg-surface-light-soft py-16 md:py-24"
      aria-labelledby="partnership-readiness-heading"
    >
      <SectionHeader
        id="partnership-readiness-heading"
        title="Preparing the foundation for responsible partnerships."
        description="Tropicash partnership capabilities are being developed in phases as the platform moves from public corporate presence toward broader merchant, developer, and financial partner readiness."
        light
      />

      <div className="mt-12 overflow-hidden rounded-2xl border border-atlantic-100 bg-white shadow-md">
        <ul className="divide-y divide-atlantic-100">
          {readinessItems.map((item) => (
            <li
              key={item.label}
              className="flex flex-col gap-2 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-6"
            >
              <div className="flex items-center gap-3">
                <span
                  className={cn(
                    "h-2.5 w-2.5 shrink-0 rounded-full",
                    readinessStatusStyles[item.status].dot,
                  )}
                  aria-hidden="true"
                />
                <span className="text-sm font-medium text-surface-light-foreground">
                  {item.label}
                </span>
              </div>
              <span
                className={cn(
                  "inline-flex w-fit items-center rounded-full border px-3 py-0.5 text-xs font-semibold",
                  readinessStatusStyles[item.status].badge,
                )}
              >
                {item.statusLabel}
              </span>
            </li>
          ))}
        </ul>
      </div>
    </HomeSection>
  );
}
